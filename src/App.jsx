    ...
    <div className='flex items-center'>
          ...
          <div className="h-screen w-screen bg-[#202123] absolute top-0 z-50 flex flex-col justify-center items-center">
            ...
            <div className="flex justify-center items-center">
              {visionState === "thinking" && (
                <div className="flex space-x-1">
                  <span className="h-[30px] w-[30px] bg-white rounded-full animate-bounce [animation-delay:-0.3s]" />
                  <span className="h-[30px] w-[30px] bg-white rounded-full animate-bounce [animation-delay:-0.15s]" />
                  <span className="h-[30px] w-[30px] bg-white rounded-full animate-bounce" />
                </div>
              )}

              {visionState === "talking" && (
                <div className="bouncing-ball"></div> // Customize your animated ball here
              )}

              {/* Listening Mic Pulse */}
              {visionAudio && visionState === "idle" && (
                <div className="voice-dot mt-6"></div>
              )}

              {/* Idle default indicator */}
              {visionState === "idle" && !visionAudio && (
                <div className="bg-white w-[75px] h-[75px] rounded-full mt-5"></div>
              )}
            </div>
            ...
            <div className="flex gap-10 absolute bottom-0 mb-10">
              ...
              {visionAudio ? (
                <button
                  onClick={handelVAoff}
                  className="hover:bg-gray-700 rounded-full bg-[#444654] flex justify-center items-center cursor-pointer p-5"
                >
                  <img src={mic} className="invert" alt="Mic On" />
                </button>
              ) : (
                <button
                  onClick={() => {
                    unlockTTS();               // 🔓 iOS fix
                    visionVoiceAssistance();   // 🎙️ Start recognition
                    setvisionAudio(true);      // ✅ Set state manually after click
                  }}
                  className="rounded-full bg-red-400 flex justify-center items-center cursor-pointer p-5"
                >
                  <img src={micoff} className="invert" alt="Mic Off" />
                </button>
              )}
              ...
              <button
                ...
                className="rounded-full hover:bg-gray-700 bg-[#444654] flex justify-center items-center cursor-pointer p-5"
              >
                <img src={close} className="invert" alt="Close" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div className='flex items-center'>
        ...
        {arrow ? (
          ...
          <div className={`bg-[#f5f5f5] flex flex-col opacity-95 text-black transition-all duration-300 backdrop-blur-2xl w-[20vw] max-[1100px]:w-[30vw] max-[800px]:hidden max-[800px]:absolute max-[800px]:w-[300px] max-[800px]:left-[-400px] z-10 h-screen ${isDark ? '' : 'invert'}`}>
            ...
            <div onClick={handelGIT} className='bg-white rounded-2xl cursor-pointer fixed justify-center h-[82.5px] border-t w-full left-0 bottom-0 backdrop-blur-2xl shadow-md flex p-3 items-center gap-3 '>
              <img src={GIT} className='w-[30px]' alt="VF" />
              <div className='font-bold pr-5'>arijit-bit</div>
            </div>
          </div>
        ) : (
          // ✅ Small Sidebar
          <div className={`bg-[#f5f5f5] flex flex-col opacity-95 text-black transition-all duration-300 backdrop-blur-2xl w-[100px] z-10 h-screen max-[800px]:hidden border-r ${isDark ? '' : 'invert '}`}>
            ...
            <div onClick={handelGIT} className='bg-white rounded-2xl cursor-pointer fixed justify-center h-[82.5px] border-t w-full left-0 bottom-0 backdrop-blur-2xl shadow-md flex items-center gap-2'>
              <img src={GIT} className='w-[55px] cursor-pointer hover:bg-gray-300 transition-all p-3 hover:rounded-[10px]' alt="AI" />
            </div>
          </div>
        )}
      </div>
      ...
      <div onClick={handelNewArrowGo} className={`bg-[#f5f5f5] flex flex-col opacity-95 text-black transition-all duration-300 backdrop-blur-2xl w-[100px] z-10 h-screen border-r max-[600px]:hidden min-[800px]:hidden ${isDark ? '' : 'invert '}`}>
        {!newArrow && (<><div className='flex items-center mt-2 justify-center'>
          ...
          <div onClick={handelGIT} className='bg-white rounded-2xl cursor-pointer fixed justify-center h-[82.5px] border-t w-full left-0 bottom-0 backdrop-blur-2xl shadow-md flex items-center gap-2'>
            <img src={GIT} className='w-[55px] cursor-pointer hover:bg-gray-300 transition-all p-3 hover:rounded-[10px]' alt="" />
          </div></>)}
      </div>
      ...
      <div className={`bg-[#f5f5f5] flex flex-col opacity-95 text-black transition-all duration-300 backdrop-blur-2xl w-[20vw] max-[1100px]:w-[30vw] min-[800px]:hidden max-[800px]:absolute max-[800px]:w-[300px] max-[800px]:left-[-400px] z-10 h-screen ${isDark ? 'border-r' : 'invert'} ${newArrow ? "max-[800px]:left-[0px] max-[500px]:w-screen max-[500px]:pl-3" : ""} `}>
        ...
        <div onClick={handelGIT} className='bg-white rounded-2xl cursor-pointer fixed justify-center h-[82.5px] border-t w-full left-0 bottom-0 backdrop-blur-2xl shadow-md flex p-3 items-center gap-2 '>
          <img src={GIT} className='w-[30px]' alt="VF" />
          <div className='font-bold pr-5'>arijit-bit</div>
        </div>
      </div>
        ...
        <div className={`flex flex-col h-screen bg-[#f5f5f5] text-black transition-all duration-300 opacity-80 backdrop-blur-2xl border-l border-black max-[800px]:border-none ${isDark ? '' : 'invert'} ${arrow ? 'w-[calc(100vw-20vw)] max-[1100px]:w-[calc(100vw-30vw)] max-[800px]:w-[calc(100vw-100px)] max-[600px]:w-screen' : 'w-[calc(100vw-100px)] border-none max-[800px]:w-[calc(100vw-100px)] max-[600px]:w-screen'}`}>
          ...
          <div className="p-4 border-b flex justify-between items-center backdrop-blur-2xl shadow-md bg-white">
            <div className='flex items-center gap-3'>
              ...
              <h1 className={`text-xl font-semibold ${isDark ? '' : ''}`}>Friday</h1>
            </div>
            ...
            <div className="flex items-end gap-2 max-[550px]:flex-col ">
              ...
              <div className='flex justify-between max-[550px]:w-full'>
                <div className='flex gap-2 cursor-pointer'>
                  ...
                  <button onClick={handelGIT} className={`bg-white border-black border px-4 py-1 rounded-xl transition-all hover:bg-gray-900 hover:border-blue-300 cursor-pointer flex justify-center items-center  min-[550px]:hidden max-[550px]:px-3 ${isDark ? '' : 'invert'}`}>
                    <img src={GIT} alt="@" />
                  </button>
                </div>
